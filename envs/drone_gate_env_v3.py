...
from drone_dynamics import DroneDynamics, LowLevelController

class DroneGateEnv(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self, n_gates:int=5):
        super(DroneGateEnv, self).__init__()
        self.dt = 0.1
        self.max_steps = 6000
        self.current_step = 0
        ...
        self.controller = LowLevelController(self.drone,
                                             Kp_pos=np.array([2.5, 2.5, 2.5]),
                                             Kd_pos=np.array([0.2, 0.2, 0.2]),
                                             Kp_att=np.array([0.1, 0.1, 0.1]),
                                             Kd_att=np.array([0.01, 0.01, 0.01]))
        
        # Define 5 target positions arranged in a circle (for simplicity, in the XY plane with different altitudes)
        if n_gates > 1:
            angles = np.linspace(0, 2*np.pi, n_gates)[:-1]  # 5 evenly spaced angles
        else:
            angles = [0]
    ...
    def step(self, desired_acc):
        desired_acc = np.clip(desired_acc, self.action_space.low, self.action_space.high)

        # Use the low-level controller to convert the high-level action into control commands.
        control = self.controller.compute_control(np.array(desired_acc))
