...
class DroneRaceCurriculumMultiEnv_v12(gym.Env):
    ...
    def __init__(self,
                 ...
                 proximity_penalty: float = 0.6,
                 proximity_exponent: float = 2.5,
                 max_proximity_penalty: float = 1.2,
                 # Overtaking parameters:
                 enable_overtake:bool=False,
                 overtake_reward: float =0.0,
                 jitter_range:list = [0.25, 0.75]):
        super(DroneRaceCurriculumMultiEnv_v12, self).__init__()
        ...
        self.max_vel = 10.0
        self.proximity_penalty = proximity_penalty      # Base penalty for proximity violations
        self.proximity_exponent = proximity_exponent     # Quadratic scaling for distance-based penalties
        self.max_proximity_penalty = max_proximity_penalty  # Cap on total proximity penalties (2× base penalty)
        self.safety_distance = safety_distance        # Distance at which proximity penalties begin
    ...
    def setup_gates_v2(self):
        ...
        # gate_positions = [
        #     [5, -2, 1],      # Gate 1 (right)
        #     [7, 0, 2],       # Gate 2 (back right)
        #     [5, 2, 2],       # Gate 3 (back)
        #     [-2.5, -2, 0.5],  # Gate 4 (middle)
        #     [-5.0, 0, 0.5],   # Gate 5 (front left)
        #     [-2.5, 2, 1.5]    # Gate 6 (left)
        # ]
    ...
    def step(self, actions):
        ...
        for agent in self.agents:
            ...
            reward, reward_components = self.reward_function(agent, target_reached, distance, vehicle["prev_distance"], new_state, unit_rel_pos)
    ...
    def reward_function(self, agent, target_reached, distance, prev_distance, state, unit_rel_pos):
        position = state[0:3]
        velocity = state[3:6]
        roll, pitch = state[6], state[7]
        x = distance / self.max_distance
        distance_reward = 2 * (np.exp(-self.a * x) - np.exp(-self.a)) / (1 - np.exp(-self.a)) - 1
        delta_distance = prev_distance - distance
        distance_change_reward = 1.0 * delta_distance
        vel_norm = np.linalg.norm(velocity)
        ...
        proximity_penalty = self.calculate_proximity_reward(agent, position, velocity)
        overtake_reward = self.calculate_overtake_reward(agent, position, velocity)

        reward = (self.w_distance * distance_reward +
                  self.w_distance_change * distance_change_reward +
                  self.w_alignment * alignment +
                  self.w_inactivity * speed_gap - 
                  self.w_proximity * proximity_penalty +
                  self.w_overtake * overtake_reward + 
                  success - failure)
        

        reward_components = {"gate": success, "failure": -failure, "alignment": self.w_alignment * alignment,
                            "distance": self.w_distance * distance_reward, "distance_change": self.w_distance_change * distance_change_reward , 
                            "proximity": -self.w_proximity * proximity_penalty, "overtake": self.w_overtake * overtake_reward}
        
        return reward, reward_components
    

    def calculate_proximity_reward(self, agent, position, velocity):
        ...
        for other_agent in self.vehicles:
            if other_agent != agent:
                other_position = self.vehicles[other_agent]["drone"].state[:3]
                distance_to_other = np.linalg.norm(position - other_position)
                
                # Calculate relative velocity (closing speed)
                other_velocity = self.vehicles[other_agent]["drone"].state[3:6]
                rel_velocity = velocity - other_velocity
                closing_speed = max(0, -np.dot(rel_velocity, (position - other_position)) / (distance_to_other + 1e-6))
                
                # Adjust safety distance based on closing speed
                dynamic_safety_distance = self.safety_distance * (1 + 0.5 * min(1.0, closing_speed / 5.0))
                
                if distance_to_other < self.drone_collision_margin:
                    # Actual collision - use full collision penalty
                    penalty = self.collision_penalty
                    num_close_drones += 1
                elif distance_to_other < dynamic_safety_distance:
                    # Calculate proximity danger based on distance
                    danger_ratio = (dynamic_safety_distance - distance_to_other) / (dynamic_safety_distance - self.drone_collision_margin)
                    
                    # Apply non-linear scaling for smoother gradient
                    penalty = self.proximity_penalty * (danger_ratio ** self.proximity_exponent)
                    
                    # Increase penalty for head-on approaches vs. parallel paths
                    if closing_speed > 0.5:  # Only consider significant closing speeds
                        penalty *= (1 + min(1.0, closing_speed / 5.0))
        ...
        # Cap the maximum proximity penalty
        capped_proximity_penalty = min(proximity_penalty, self.max_proximity_penalty)
        
        # Scale the cap based on number of nearby drones (optional enhancement)
        if num_close_drones > 1:
            # Increase cap slightly for multiple drones, but still maintain a reasonable limit
            adjusted_cap = self.max_proximity_penalty * (1 + 0.2 * (num_close_drones - 1))
            capped_proximity_penalty = min(proximity_penalty, adjusted_cap)


        return capped_proximity_penalty
