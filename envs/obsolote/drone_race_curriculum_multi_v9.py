...
class DroneRaceCurriculumMultiEnv_v9(gym.Env):
    ...
    def __init__(self,
                 ...
                 gate_collision_margin: float = 0.5,
                 gate_passing_tolerance: float = 0.5,
                 enable_takeover:bool=False,
                 takeover_reward: float =0.0,
                 jitter_range:list = [0.25, 0.75],
                 close_start_probability: float = 0.3):
        super(DroneRaceCurriculumMultiEnv_v9, self).__init__()
        ...
        self.gate_collision_margin = gate_collision_margin  # Gate Collision threshold in meters.
        ...
        self.close_start_probability = close_start_probability  # Probability of starting close to gates.
        self.init_strategy = init_strategy        # Initialization strategy for training.
        ...
        for agent in self.agents:
            ...
            self.vehicles[agent] = {
                ...
                "distance_to_gate": 1e6,   # distance to the current target.
        ...
        self.setup_gates(self.n_gates, self.radius)
        ...
        reward_functions = {
            1: self.reward_function_1,
            2: self.reward_function_2,
            3: self.reward_function_3,
            4: self.reward_function_4,
            5: self.reward_function_5,
            6: self.reward_function_6
        }

        if reward_type in reward_functions:
            self.reward_function = reward_functions[reward_type]
        else:
    ...
    def check_gate_collisions(self, drone_id, position):
        """
        Efficiently check for collisions between a drone and gates.
        Only checks gates that are close enough to possibly collide.
        ...
        Returns:
            collision_occurred: Boolean indicating if collision happened
        """
        # Current target gate - special handling
        current_gate_idx = self.vehicles[drone_id]["current_target_index"]
        current_gate = self.gates[current_gate_idx]
        
        # First check if passing through current target gate correctly
        if self.check_gate_passing(position, current_gate):
            # If passing correctly, don't check this gate for collision
            exclude_gate = current_gate_idx
        else:
            # Otherwise, include all gates in check
            exclude_gate = None
        
        # Calculate distances to all gates
        gates_with_distances = []
        for idx, gate in enumerate(self.gates):
            if idx == exclude_gate:
                continue  # Skip the excluded gate
                
            # Calculate distance to gate
            gate_pos = gate['center']
            distance = np.linalg.norm(position - gate_pos)
            
            # Only consider gates within possible collision distance
            gate_size = max(gate['width'], gate['height'])
            if distance < gate_size + self.gate_collision_margin + 0.5:  # Add some margin
                gates_with_distances.append((idx, distance))
        
        # Sort gates by distance (check closest first)
        gates_with_distances.sort(key=lambda x: x[1])
        
        # Only check a limited number of gates for efficiency
        max_gates_to_check = min(3, len(gates_with_distances))
        
        for i in range(max_gates_to_check):
            if i >= len(gates_with_distances):
                break
                
            gate_idx = gates_with_distances[i][0]
            gate = self.gates[gate_idx]
            
            if self.check_drone_gate_collision(position, gate):
                return True  # Collision detected
        
        return False  # No collision

    def check_drone_gate_collision(self, drone_position, gate):
        """
        Check for collision between drone and gate frame.
        Optimized for accuracy and efficiency.
        
        Args:
            drone_position: 3D position of drone
            gate: Gate information
            
        Returns:
            collision: Boolean indicating if collision occurred
        """
        # Quick distance check first (most efficient)
        distance_to_gate = np.linalg.norm(drone_position - gate['center'])
        gate_diagonal = np.sqrt(gate['width']**2 + gate['height']**2) / 2
        
        # Skip detailed check if drone is clearly too far away
        if distance_to_gate > gate_diagonal + self.gate_collision_margin:
            return False
        
        # Transform to gate's local coordinate system
        local_position = self.transform_to_gate_coordinates(drone_position, gate)
        
        # Unpack gate dimensions
        gate_width = gate['width']
        gate_height = gate['height']
        half_width = gate_width / 2
        half_height = gate_height / 2
        frame_thickness = 0.1
        
        # Skip collision check if drone is passing properly through gate
        # This is the "safe zone" inside the gate
        if (abs(local_position[0]) < self.gate_passing_tolerance and
            abs(local_position[1]) < half_width and
            abs(local_position[2]) < half_height):
            return False
        
        # Only continue if drone is close to the gate plane
        if abs(local_position[0]) > self.gate_collision_margin:
            return False
        
        # Check collision with horizontal frame parts (top and bottom)
        if abs(local_position[1]) < half_width:  # Within gate width
            dist_to_top = abs(local_position[2] - half_height)
            dist_to_bottom = abs(local_position[2] + half_height)
            if (dist_to_top < frame_thickness + self.gate_collision_margin or
                dist_to_bottom < frame_thickness + self.gate_collision_margin):
                return True
        
        # Check collision with vertical frame parts (left and right)
        if abs(local_position[2]) < half_height:  # Within gate height
            dist_to_right = abs(local_position[1] - half_width)
            dist_to_left = abs(local_position[1] + half_width)
            if (dist_to_right < frame_thickness + self.gate_collision_margin or
                dist_to_left < frame_thickness + self.gate_collision_margin):
                return True
        
        # Check for frame corner collisions 
        # (this is an extra check that improves accuracy near corners)
        corners = [
        ...
        for corner in corners:
            # Calculate distance to corner in the YZ plane
            dist_to_corner = np.sqrt((local_position[1] - corner[0])**2 + 
                                    (local_position[2] - corner[1])**2)
            if dist_to_corner < frame_thickness + self.gate_collision_margin:
                return True
    ...
    def step(self, actions):
        ...
        for agent in self.agents:
            ...
            vehicle["distance_to_gate"] = distance
            if distance > 1e-6:
            ...
            if self.reward_type == 1 or self.reward_type == 2:
                reward = self.reward_function(distance, vehicle["prev_distance"], velocity, unit_rel_pos)
            elif self.reward_type == 3:
                reward = self.reward_function(distance, vehicle["prev_distance"], velocity, unit_rel_pos, opponents_info, position)
            elif self.reward_type == 4 or self.reward_type == 5 or self.reward_type == 6:
                reward = self.reward_function(agent, distance, vehicle["prev_distance"], velocity, unit_rel_pos, position)
            else:
        ...
        if self.enable_collision:
            ...
            for i, (drone_i, pos_i) in enumerate(positions.items()):
                ...
                # Check for collisions with gates (optimized)
                if not collision_occurred:  # Skip if already in collision with another drone
                    if self.check_gate_collisions(drone_i, pos_i):
                        # Apply collision penalty
                        rewards[drone_i] -= self.collision_penalty
                        infos[drone_i]["collision"] = 1
                        if self.terminate_on_collision and drone_i == self.training_agent:
                            dones["__all__"] = True
    ...
    def reward_function_4(self, agent, distance, prev_distance, velocity, unit_rel_pos, position):
        """
        Simplified reward function with clear, separable components.
        Ensures all returns are scalar values, not arrays.
        
        Args:
            agent: ID of the agent being rewarded
            distance: Current distance to target gate
            prev_distance: Previous distance to target gate
            velocity: Current velocity vector of the drone
            unit_rel_pos: Unit vector pointing from drone to gate
            position: Current 3D position of the drone
            
        Returns:
            reward: The calculated reward value (scalar)
        """
        # Initialize total reward as scalar
        reward = 0.0  
        
        # --- 1. DISTANCE REWARD: Reward progress toward target gate ---
        # Ensure scalar values
        distance = float(distance)
        prev_distance = float(prev_distance)
        
        # Simple linear reward for getting closer to the gate
        delta_distance = prev_distance - distance  # Positive when getting closer
        distance_reward = 2.0 * delta_distance     # Scale the reward
        reward += float(distance_reward)
        
        # --- 2. VELOCITY REWARD: Encourage maintaining minimum speed ---
        # Convert velocity to scalar speed
        vel_norm = float(np.linalg.norm(velocity))
        velocity_reward = 0.0
        
        # Simple threshold-based velocity reward
        if vel_norm < self.minimum_velocity:
            # Penalty for being too slow
            velocity_reward = -0.2 * (self.minimum_velocity - vel_norm)
        
        reward += float(velocity_reward)
        
        # --- 3. DIRECTION REWARD: Encourage flying toward the gate ---
        direction_reward = 0.0
        if vel_norm > 0.5:  # Only check direction if drone is moving
            # Measure alignment between velocity and direction to gate
            # Ensure dot product is scalar
            dot_product = float(np.dot(velocity, unit_rel_pos))
            cos_angle = dot_product / vel_norm
            # Convert to a reward between -0.5 and 0.5
            direction_reward = 0.5 * cos_angle
        
        reward += float(direction_reward)
        
        # --- 4. COLLISION AVOIDANCE REWARD: Penalize getting too close to other drones ---
        collision_reward = 0.0
        
        # Only calculate if collision detection is enabled
        if getattr(self, 'enable_collision', False):
            for other_agent in self.vehicles:
                if other_agent != agent:
                    other_position = self.vehicles[other_agent]["drone"].state[0:3]
                    # Ensure scalar distance
                    distance_to_other = float(np.linalg.norm(position - other_position))
                    
                    # Define collision margin with fallback
                    margin = getattr(self, 'drone_collision_margin', 0.5) * 2
                    
                    # Simple proximity penalty
                    if distance_to_other < margin:
                        # Stronger penalty as drones get closer
                        collision_reward = -0.5 * (1.0 - distance_to_other/margin)
                        break  # Only count closest drone
        
        reward += float(collision_reward)
        
        # --- DEBUG INFORMATION ---
        # Print components for debugging
        # print(f"Agent {agent} rewards: dist={distance_reward:.3f}, vel={velocity_reward:.3f}, dir={direction_reward:.3f}, col={collision_reward:.3f}, total={reward:.3f}")
        
        # Ensure final reward is a Python scalar, not numpy array
        final_reward = float(np.clip(reward, -1.0, 1.0))

        # print ("final_reward", final_reward)
        
        # Verify type before returning
        if not isinstance(final_reward, float):
            print(f"Warning: Reward not float: {final_reward}, type: {type(final_reward)}")
            final_reward = float(final_reward)
            
        return final_reward
    
    def reward_function_6(self, agent, distance, prev_distance, velocity, unit_rel_pos, position):
        """
        Enhanced reward function that encourages gate-aware lane formation and 
        includes race position awareness to improve racing efficiency.
        """
        # --- CORE RACING REWARD ---
        # Progress reward (main driver of behavior)
        distance = float(distance)
        prev_distance = float(prev_distance)
        delta_distance = prev_distance - distance
        progress_reward = 2.0 * delta_distance
        ...
        # Define optimal velocity range
        optimal_velocity_lower = 0.8 * self.minimum_velocity
        optimal_velocity_upper =  1.5 * self.minimum_velocity

        # Basic velocity penalty when too slow
        if vel_norm < self.minimum_velocity:
            velocity_reward = -0.2 * (self.minimum_velocity - vel_norm)
        # Reward for staying in optimal range
        elif vel_norm <= optimal_velocity_upper:
            # Max reward at middle of optimal range
            optimal_center = (optimal_velocity_lower + optimal_velocity_upper) / 2
            velocity_reward = 0.3 * (1.0 - abs(vel_norm - optimal_center) / 
                                    (optimal_velocity_upper - optimal_velocity_lower))
        # Small penalty for excessive speed
        else:
            velocity_reward = -0.1 * ((vel_norm - optimal_velocity_upper) / 
                                    optimal_velocity_upper)
        ...
        if getattr(self, 'enable_collision', False):
            for other_agent in self.vehicles:
                if other_agent != agent:
                    ...
                    # Simplified collision avoidance with two zones
                    safe_distance = getattr(self, 'drone_collision_margin', 0.5) * 3.0
                    critical_distance = getattr(self, 'drone_collision_margin', 0.5) * 1.5
                    
                    if distance_to_other < safe_distance:
                        if distance_to_other < critical_distance:
                            # Critical zone - stronger penalty
                            proximity_factor = (critical_distance - distance_to_other) / critical_distance
                            collision_reward = -1.0 * proximity_factor**2
                        else:
                            # Warning zone - lighter penalty
                            normalized_dist = (distance_to_other - critical_distance) / (safe_distance - critical_distance)
                            collision_reward = -0.3 * (1.0 - normalized_dist)
                        break  # Only consider closest drone
        
        # --- GATE-AWARE LANE FORMATION REWARD ---
        lane_reward = 0.0
        
        # Only calculate if multiple drones are present
        if getattr(self, 'enable_collision', False) and len(self.vehicles) > 1:
            current_target_idx = self.vehicles[agent]["current_target_index"]
            
            # Get the gate's position
            gate_position = self.gate_positions[current_target_idx]
            gate_yaw = self.gates[current_target_idx]["yaw"]
            
            # Initialize perpendicular vectors for lane formation
            # Default vectors if gate orientation is not available
            race_direction = gate_position - position
            race_direction = race_direction / (np.linalg.norm(race_direction) + 1e-10)  # Avoid division by zero

            
            # Create perpendicular vectors based on gate orientation
            gate_forward = np.array([np.cos(gate_yaw), np.sin(gate_yaw), 0])
            gate_right = np.array([-np.sin(gate_yaw), np.cos(gate_yaw), 0])
            gate_up = np.array([0, 0, 1])
            
            # Use these as your perpendicular plane
            right_vector = gate_right
            up_perp = gate_up
            
            # --- RACE POSITION AWARENESS ---
            # Check if this drone is ahead in the race
            drone_ahead = True
            drones_ahead_count = 0
            drones_behind_count = 0
            
            for other_agent in self.vehicles:
                if other_agent == agent:
                    continue
                    
                if self.vehicles[other_agent]["current_target_index"] > self.vehicles[agent]["current_target_index"]:
                    drone_ahead = False
                    drones_ahead_count += 1
                elif self.vehicles[other_agent]["current_target_index"] == self.vehicles[agent]["current_target_index"]:
                    other_distance = self.vehicles[other_agent]["distance_to_gate"]
                    if other_distance < distance:  # Other drone is closer to gate
                        drone_ahead = False
                        drones_ahead_count += 1
                    else:
                        drones_behind_count += 1
                else:
                    drones_behind_count += 1
            
            # Find drones targeting the same gate and calculate lane separations
            same_gate_drones = []
            other_positions = []
            ...
            for other_agent in self.vehicles:
                ...
                # Consider drones targeting same or adjacent gates (more relevant for lane formation)
                other_idx = self.vehicles[other_agent]["current_target_index"]
                if abs(other_idx - current_target_idx) <= 1:
                    same_gate_drones.append(other_agent)
                    other_positions.append(self.vehicles[other_agent]["drone"].state[0:3])
            
            # If there are other drones to form lanes with
            if same_gate_drones:
                # Calculate lateral separations in the right and up directions
                min_lateral_separation = float('inf')
                
                for other_pos in other_positions:
                    # Vector from drone to other drone
                    separation_vector = other_pos - position
                    
                    # Project onto racing direction to get longitudinal component
                    longitudinal = np.dot(separation_vector, race_direction)
                    
                    # Ignore drones that are far behind or far ahead (not lane competitors)
                    if abs(longitudinal) > 8.0:  # Adjustable threshold
                        continue
                    
                    # Calculate lateral separation components
                    lateral_right = np.dot(separation_vector, right_vector)
                    lateral_up = np.dot(separation_vector, up_perp)
                    
                    # Overall lateral distance (Euclidean in the perpendicular plane)
                    lateral_distance = np.sqrt(lateral_right**2 + lateral_up**2)
                    
                    # Track minimum separation among relevant drones
                    if lateral_distance < min_lateral_separation:
                        min_lateral_separation = lateral_distance
                
                # If we found relevant lateral separations
                if min_lateral_separation != float('inf'):
                    # Define optimal lane separation
                    optimal_lane_width = 2.0  # Adjustable parameter
                    
                    if min_lateral_separation < optimal_lane_width:
                        # Encourage more separation when too close laterally
                        lane_reward = 0.4 * (min_lateral_separation / optimal_lane_width)
                    else:
                        # Max reward when at optimal separation or greater
                        lane_reward = 0.4
                        
                    # Special case: penalize severely if drones are almost on top of each other
                    if min_lateral_separation < 0.5:
                        lane_reward = -0.2
                    
                    # Race position awareness affects preferred lane position
                    if drone_ahead:
                        # If drone is leading, give small preference to inside lane (racing line)
                        # This helps race leaders optimize their racing line
                        lane_prefer_factor = min(1.0, min_lateral_separation / optimal_lane_width)
                        
                        # Stronger preference for inside lane when clearly ahead
                        if drones_behind_count >= 2:
                            lane_reward += 0.15 * (1.0 - lane_prefer_factor)
                        else:
                            lane_reward += 0.1 * (1.0 - lane_prefer_factor)
                    else:
                        # If drone is behind, slightly encourage outside lane for overtaking
                        # This helps trailing drones find alternative paths
                        if drones_ahead_count >= 2:
                            # Multiple drones ahead - encourage more significant deviation
                            lane_reward += 0.15 * min(1.0, min_lateral_separation / (optimal_lane_width * 1.2))
                        else:
                            # At least one drone ahead - minor encouragement for outside lane
                            lane_reward += 0.1 * min(1.0, min_lateral_separation / optimal_lane_width)
        
        # Adjust lane reward based on collision risk
        if collision_reward < -0.5:  # In moderate to high collision risk
            # Reduce lane reward's influence as collision risk increases
            lane_adjustment_factor = np.clip(1.0 + collision_reward, 0.0, 1.0)
            lane_reward *= lane_adjustment_factor
        
        # Normalize progress (typically ranges from -0.5 to 0.5)
        normalized_progress_reward = np.clip(progress_reward / 2.0, -1.0, 1.0)

        # Normalize velocity (typically ranges from -0.5 to 0.5)
        normalized_velocity_reward = np.clip(velocity_reward / 0.5, -1.0, 1.0)

        # Direction component is already normalized (-0.5 to 0.5)
        normalized_direction_reward = direction_reward

        # Collision is already normalized (-1.0 to 0)
        normalized_collision_reward = collision_reward

        # Lane reward normalization
        normalized_lane_reward = np.clip(lane_reward / 0.4, -1.0, 1.0)

        # Balanced weights (should sum to approximately 1.0)
        final_reward = (
            0.5 * normalized_progress_reward +  # Progress is primary
            0.15 * normalized_velocity_reward +
            0.15 * normalized_direction_reward +
            0.1 * normalized_collision_reward +
            0.1 * normalized_lane_reward
        )

        if hasattr(self, 'debug_rewards') and self.debug_rewards:
            print(f"Agent {agent}: prog={normalized_progress_reward:.2f}, vel={normalized_velocity_reward:.2f}, "
                f"dir={normalized_direction_reward:.2f}, coll={normalized_collision_reward:.2f}, "
                f"lane={normalized_lane_reward:.2f}, total={final_reward:.2f}")

        return final_reward

    def reward_function_5(self, agent, distance, prev_distance, velocity, unit_rel_pos, position):
        """
        Simplified but effective reward function balancing racing and safety.
        """
        # --- CORE RACING REWARD ---
        # Progress reward (main driver of behavior)
        distance = float(distance)
        ...
        # --- COMBINE REWARDS ---
        # Simple weighted sum - weights adjusted based on curriculum stage
        # When collision_penalty parameter is higher, collision avoidance becomes more important
        collision_weight = getattr(self, 'collision_penalty', 0.5)
        progress_weight = 1.0
        
        # Simple adaptive mechanism - reduce progress reward when collision is imminent
        if collision_reward < -0.8:  # Very close to collision
            progress_weight = 0.7  # Temporarily reduce progress importance
        ...
        # Ensure reward is a Python scalar
